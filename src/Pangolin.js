...
class Pangolin {
  ...
  async swapAvaxToUSDT(){
    ...
    const to = window.web3.utils.toChecksumAddress(this.metaMask.account) // should be a checksummed recipient address
    ...
    var myContract1 = new window.web3.eth.Contract(IPangolinERC20.abi, tokenB.address, {
      from: this.metaMask.account, // default from address
    });

    var allowance = await myContract1.methods.allowance(this.metaMask.account,this.FujiRouter).call();
    ...
    //myContract.swapExactAVAXForTokens.call(amountOut,this.metaMask.account,this.metaMask.account,1000);
  }
  ...
  setupSwap(){
    ...
    this.swapAvaxButton.addEventListener('click', () => {
      if(this.metaMask.account !== undefined){
        this.swapAvaxToUSDT();
      }
    });
  }
